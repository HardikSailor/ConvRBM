function test_ReLU_RAW_CRBM_hardik_main(ws, num_bases, spacing, pbias, pbias_lb, pbias_lambda, epsilon, l2reg, epsdecay)
...
dataname = 'TIMIT';
batch_size = 1;
filelist = dir('F:\Database_collection\timit_na_SA\*.wav');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%lambda = 0.9;
CD_mode = 'mf';
bias_mode = 'simple'; %%% sparsity regularization
...
fname_prefix = sprintf('TIMIT_raw_speech_sigmoid_128_16_40_LB_%s_V1b_w%d_b%02d_pc%d_sigpc%g_p%g_pl%g_plambda%g_sp%d_%s_eps%g_epsdecay%g_l2reg%g_bs%02d_%s', dataname, ws, num_bases, pbias, pbias_lb, pbias_lambda, spacing, CD_mode, epsilon, epsdecay, l2reg, batch_size, datestr(now, 30));
...
%                    fprintf('Processing %s\n', filelist(j).name);
        fname=strcat('F:\Database_collection\timit_na_SA\',filelist(j).name);
        ...
        [ferr, dW, dh, dv, poshidactvalues, poshidsample, negdata, stat]= fobj_tirbm_CD_LB_sparse_audio(imdatatr, W, hbias_vec, vbias_vec, pars, CD_mode, bias_mode, spacing, l2reg,momentum);
...
function [ferr dW_total dh_total dv_total poshidactvalues poshidsample negdata stat] = ...
    fobj_tirbm_CD_LB_sparse_audio(imdata, W, hbias_vec, vbias_vec, pars, CD_mode, bias_mode, spacing, l2reg,momentum)

ws = size(W,1);
 [poshidactvalues,poshidsample] = test_sigmoid_ConvRBM_sample_multrand_1d(imdata, W, hbias_vec, pars);

% poshidexp= ConvRBM_inference_fixconv_1d(imdata, W, hbias_vec, pars);
% %   imagesc(poshidexp);figure;
% [poshidstates poshidactvalues] = ConvRBM_sample_multrand_1d(poshidexp, spacing);
%  if strcmp(CD_mode, 'mf'), poshidstates = poshidactvalues; end
%     imagesc(poshidstates);colormap('jet');figure;
posprods = ConvRBM_vishidprod_fixconv_1d(imdata, poshidactvalues, ws);
poshidact = squeeze(sum(poshidactvalues,2));
posvisact = squeeze(sum(imdata,1));

% neghidprobs = poshidactvalues;
% poshidstates = poshidactvalues ;
for j=1:pars.K_CD
    negdata = ConvRBM_reconstruct_LB_fixconv_1d(poshidsample, W, pars);
...
%     [neghidstates neghidprobs] = my_tirbm_sample_multrand_1d(neghidexp, spacing);
    ...
    [neghidactvalues,neghidsample] = test_sigmoid_ConvRBM_sample_multrand_1d(negdata, W, hbias_vec, pars);
end
negprods = ConvRBM_vishidprod_fixconv_1d(negdata, neghidactvalues, ws);
neghidact = squeeze(sum(neghidactvalues,2));
...
% meanActiv = lembda*meanActiv+(1-lembda)*(mean(mean(poshidactvalues)));
% dsh = meanActiv-sparseTarget;
% numcases1 = size(poshidactvalues,1)*size(poshidactvalues,2);
% numcases1 = 1;
% numcases2=1;
