function RAW_CRBM_hardik_main(ws, num_bases, spacing, pbias, pbias_lb, pbias_lambda, epsilon, l2reg, epsdecay)
% clear all; close all; clc;
% addpath ../voicebox
% addpath F:\Deep_Learning\Tools\crbm_audio\voicebox;
dataname = 'TIMIT';
batch_size = 1;
filelist = dir('H:\TIMIT_NO_SA\*.wav');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  load wave2_all;
%          wave_mean =  mean(wave2_all);
%          wave_var =  var(wave2_all);
%  load Wave_stats_TIMIT_no_SA;
%  ensamble_mean = mean(cat(1, Means_All{:}));
%  ensamble_var= mean(cat(1, Var_All{:}));
%  clear Means_All Var_All;
...
fname_prefix = sprintf('2000_raw_speech_128_16_80_LB_%s_V1b_w%d_b%02d_pc%d_sigpc%g_p%g_pl%g_plambda%g_sp%d_%s_eps%g_epsdecay%g_l2reg%g_bs%02d_%s', dataname, ws, num_bases, pbias, pbias_lb, pbias_lambda, spacing, CD_mode, epsilon, epsdecay, l2reg, batch_size, datestr(now, 30));
...
for t=1:num_trials
    ...
    for j=veclist;
        tt1 = tt1+1;
        %         disp(j)
        %          rand_j = floor((length(filelist))*rand(1));randi(numel(filelist))
        fprintf('.');
        %            fprintf('Processing %s\n', filelist(rand_j).name);
        fname=strcat('H:\TIMIT_NO_SA\',filelist(j).name);
        [sig,fs]=wavread(fname);
        sig = pre_processing(sig,fs);
        %            sig = (sig-ensamble_mean)./sqrt(ensamble_var);
        sig = bsxfun(@minus, sig,mean(sig));
        sig = bsxfun(@rdivide, sig,(std(sig)));
        %                 sig = (sig-min(sig))./(max(sig)-min(sig));
        %
        imdata = hardik_trim_audio_for_spacing_fixconv(sig, ws, spacing);
        imdatatr = imdata;
        if t<5,
        ...
        % update parameters
        %         Winc = momentum*Winc + (1-momentum)*epsilon*dW;
...
[poshidexp,poshidprobs] = tirbm_inference_fixconv_1d(imdata, W, hbias_vec, pars);
%   imagesc(poshidexp);figure;
%  [poshidstates poshidprobs2] = tirbm_sample_multrand_1d(poshidexp, spacing);
%  if strcmp(CD_mode, 'mf'), poshidstates = poshidprobs; end
%  imagesc(poshidprobs,[0 1]);colormap('gray');figure;
...
for j=1:pars.K_CD
    negdata = tirbm_reconstruct_LB_fixconv_1d(poshidprobs, W, pars);
    [neghidexp,neghidprobs] = tirbm_inference_fixconv_1d(negdata, W, hbias_vec, pars);
%      [neghidstates neghidprobs2] = tirbm_sample_multrand_1d(neghidexp, spacing);
